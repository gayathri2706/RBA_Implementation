...
def process_mcie_dditive_etl(line_id, from_date, to_date, connection):
    ...
    def get_component_id_mcie(dt, prod_data, tolerance_minutes=45, debug=False):
        """
    ...
    matched_df['component_id'] = matched_df['datetime'].apply(
        lambda x: get_component_id_mcie(x, prod_data, tolerance_minutes=45, debug=False))
    matched_df['mixer_name'] = config['Mixer Name']
    ...
    df['timestamp'] = df.apply(compute_actual_datetime_mcie, axis=1)
    df['date'] = df['date'].dt.date
...
def compute_actual_datetime_mcie(row):
    base_date = row['date']
    time = row['time']
    shift = row['shift']
    base_datetime = datetime.combine(base_date, time)
